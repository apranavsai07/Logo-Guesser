...
async function downloadMissing() {
  for (const logo of missing) {
    if (logo.name === 'Java') {
        ...
        if (!fs.existsSync(`public/logos/${logo.id}.svg`)) {
    ...
    if (slug) {
        let res = await fetch(`https://raw.githubusercontent.com/simple-icons/simple-icons/develop/icons/${slug}.svg`);
        if (!res.ok && slug === 'windows') {
            res = await fetch(`https://raw.githubusercontent.com/simple-icons/simple-icons/develop/icons/windows11.svg`);
        }
        if (!res.ok && slug === 'lovable') {
            // Lovable isn't in simpleicons. Let's make a generic heart.
            ...
            console.log(`Created fallback for Lovable`);
            continue;
        }
        
        if (res.ok) {
            fs.writeFileSync(`public/logos/${logo.id}.svg`, await res.text());
            console.log(`Downloaded ${logo.name} (${slug}) from github`);
        } else {
